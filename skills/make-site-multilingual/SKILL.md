...
- **The `data-editable="array-item"` element must be the item component's root** _(RCC layer)_. One level in, CloudCannon's re-render nests a second copy of the component's root inside the region and duplicates its `data-rosey-ns` — the Visual Editor shows source text while the built site translates correctly ([§3g](tagging.md#rule-the-array-item-region-element-must-be-the-components-own-root-rcc-layer)).
